...
function clone(){
	...
	sudo chmod -R 777 $webroot/dvwa
        ...
        sed -i '17 c $_DVWA[ 'db_user' ] = "'root'";' $webroot/dvwa/config/config.inc.php
        sed -i '18 c $_DVWA[ 'db_password' ] = "'root'";' $webroot/dvwa/config/config.inc.php

        #creating database
        echo "Creating dvwa database"
        mysql -u root -p root -e "CREATE DATABASE IF NOT EXISTS dvwa"
